...
## Оглавление
 ...
 [Настройки Домино](#Настройки-Домино) 
 [Миграция](#Миграция) 
 [TODO](#TODO)
...
- Сейчас уведомительный по ФФД 1.05
- Срок - как можно быстрее, честный знак работает с ошибками
- Оптимизация затрат на обслуживание и поддержку
- Бесшовная интеграция, обеспечение одновременной работы старых и новых касс 
- Внедрение системы лояльности (2й этап)
...
Используем резервные номера, чтобы нумерация кассовых лент начиналась с нуля
иначе старые переакцептуются

1. Касса1 живая Пилот               - > Выбыла
2. Касса2 резерв Фронтол            - > живая Фронтол
3. Касса3 резерв тестовая Фронтол   - > живая Фронтол
4. Касса4 живая Пилот               - > Выбыла
5. Касса5 резерв Фронтол            - > живая Фронтол
6. Касса6 живая Пилот               - > Выбыла
...
- frontol_converter_run.bat для каждой кассы свой
...
## Настройки Фронтол
    - Настройка операционных систем из руководства интегратора Атол для Win 10
        - обязательно, иначе шедулер отваливается
    ...
    - Глобальный контроль 
    - Обмен
        - Входящий канал Атол (локальная папка) 
        - Исходящий канал пилот (локальная папка)
![Настройка номера кассы](images/1_settings_kas_number.png)

[:arrow_up:Оглавление](#Оглавление)

## Настройки Домино
    - Справочник партнеров -> Кассовые аппараты -> ПО для кассового аппарата
![Настройка версии кассы](images/2_settings_kas_soft.png)

    - W:\PROJECT\volga2\CLASSIF
    - удалять строки с уже существующим индексом нельзя, только добавлять (пострадает ссылочная целостность БД)
    - код Frontol должен соответствовать справочнику фронтола  
![Настройка видов маркированного товара](images/3_settings_mark_type.png)    

    - для пакетного изменения в справочнике в заказах поствавщику доступен следующий функционал:
        - создать .csv следующего формата
        - "341018;1;0" где "код; количество(1); тип маркировки"
![Пакетное изменение признака маркировки](images/4_mark_batch_import.png)     
...
- [X] Кассиры доступ и прочее
    - [X] Инкассация при закрытии смены
        драйвер - параметры ККТ - функция не поддерживается в нашей версии
        доступна только на старых прошивках
    - [X] Шрифты четче
    - [X] Шрифты крупнее
    - [X] Новая раскладка клавиатуры
    - [X] Ключ клавиатуры и доступ, разобраться с концепцией для Touchscreen  
    - [X] Обновить прошивки на кассах  
    - [X] Шаблон копии чека
    - [X] Шаблон товарного чека 
    - [X] Реквизит RRN со слипа где брать? (решено через ЛК Сбербизнес)
    - [X] Прикрутить логотип на чек
    - [X] Округление до рубля вверх при частичном возврате за наличные    
    - [X] Прикрутить логотип на дисплей покупателя
        - Настройки → Системные → РМ и оборудование → Дисплей покупателя
    - [X] Доступ в ЛК ОФД кассирам (ограниченный поиск чека)
    - [X] Доступ в ЛК Сбербизнес (ограниченный поиск RRN)
    

- [X] Собрать требования к списку избранных товаров
...
- [X] Тестирование загрузка
    ...
    - [X] Дисконтные карты/классификаторы пакетом, проброс через папку источник        
...
- [ ] Тестирование регистрация
    ...
    - [X] Вернуть мерный товар
    - [X] Пробить обычный товар как мерный - ждем ошибку
    - [X] Вернуть старый товар по свободной цене (только ответственный)
    - [X] Пробить маркированный товар
    - [X] Пробить маркированный товар повторно - ждем ошибку    
    - [X] Вернуть маркированный товар
    - [X] Вернуть маркированный товар повторно - ждем ошибку
    - [X] Автономный режим Пробить маркированный товар (нужны права на ОИСМ)
    - [X] Автономный режим Пробить маркированный товар повторно - ждем ошибку    
    - [X] Автономный режим Вернуть маркированный товар
    - [X] Автономный режим Вернуть маркированный товар повторно - ждем ошибку
    - [X] Пробить мультиоплату
    - [X] Возвраты все    
...
- [X] Тестирование снятие касс
    ...
    - [ ] Скидки показывает? C поле 9 Номер карты обрезает фронтол, 
...
- [ ] Доработки
    ...
    - [X] Заполнить в бд поле текст тоже, иначе шаблоны некорректные 
    - [X] Перенос настроек БД с кассы на кассу как?       
    - [ ] Пофиксить работу фронтола с путями до сетевой папки Домино   

- [ ] Доработки 2й этап   
    - [ ] Подумать про мониторинг
        - Отловить неработающий шедулер для включённой кассы
